            ...
            for (int i = 0; i < UserList.size(); i++) {
                if (PropertyList.get(index).getRepresentativeID().equals(UserList.get(i).getUserID())) {
                    ...
                    for (int j = 0; j < AdminList.size(); j++) {
                        if (PropertyList.get(index).getRepresentativeID().equals(AdminList.get(j).getAdminID())) {
                            this.repID = new SimpleStringProperty(AdminList.get(j).getAdminID());
                        }
                    }
                }
            }
